...
export class UserController {
  ...
  public async searchUserByCredentials(@Payload() searchParams: {
    email: string;
    password: string;
  }, @Ctx() context: RmqContext): Promise<IUserSearchResponse> {
    ...
    if (searchParams.email && searchParams.password) {
      ...
      if (user && user[0]) {
       ...
       if (await bcrypt.compare(searchParams.password, user[0].password)) {  
          result = {
  ...
  public async getUserById(@Payload() id: string, @Ctx() context: RmqContext): Promise<IUserSearchResponse> {
    let result: IUserSearchResponse;
  ...
  public async confirmUser(@Payload() confirmParams: {
    link: string;
  }, @Ctx() context: RmqContext): Promise<IUserConfirmResponse> {
  ...
  public async createUser(@Payload() userParams: IUser,@Ctx() context: RmqContext): Promise<IUserCreateResponse> {
    let result: IUserCreateResponse;

    console.log(`create_user : ${userParams.email}`);
